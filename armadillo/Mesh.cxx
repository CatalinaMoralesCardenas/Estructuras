...
// -------------------------------------------------------------------------
float* Mesh::
getNormal( unsigned long j )
{
  return( this->m_Norma.data( ) + ( j * 3 ) );
}
...
// -------------------------------------------------------------------------
const float* Mesh::
getNormal( unsigned long j ) const
{
  return( this->m_Norma.data( ) + ( j * 3 ) );
}
...
// -------------------------------------------------------------------------
void Mesh::
getNormal( unsigned long j, float* n ) const
{
  std::memcpy( n, this->getNormal( j ), 3 * sizeof( float ) );
}
...
// -------------------------------------------------------------------------
void Mesh::
getNormal( unsigned long j, double* n ) const
{
  const float* fn = this->getNormal( j );
  n[ 0 ] = double( fn[ 0 ] );
  n[ 1 ] = double( fn[ 1 ] );
  n[ 2 ] = double( fn[ 2 ] );
}
...
{
  this->m_Norma.push_back( x );
  this->m_Norma.push_back( y );
  this->m_Norma.push_back( z );
...
{
  this->m_Norma.push_back( float( x ) );
  this->m_Norma.push_back( float( y ) );
  this->m_Norma.push_back( float( z ) );
  return( this->getNumberOfNormals( ) - 1 );
}
...
addNormal( float* n )
{
  return( this->addNormal( n[ 0 ], n[ 1 ], n[ 2 ] ) );
}
...
_strIn( std::istream& in )
{
  ...
  while( !in.eof( ) )
  {
    ...
    if( cmd == "v" || cmd == "V" )
    {
      ...
      //std::cout<<"p: "<<this->m_Geometry.size();
    }
